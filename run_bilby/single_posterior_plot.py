...
def plot_posterior(path_json, path_outdir, default_plot_parameters=True, show_fig=False, plot_kde=False):
    ...
    """read json file"""
    file_path_ver1 = path_json
    data_ver1 = read(file_path_ver1, package='core')
    with open(file_path_ver1, 'r') as f:
        others_data = json.load(f)

    """set version1 posterior"""
    #posterior_samples_ver1_origin = data_ver1.samples_dict.to_pandas()
    #posterior_samples_ver1 = conversion_samples(posterior_samples_ver1_origin)
    posterior_samples_ver1 = data_ver1.samples_dict.to_pandas()
    
    """set injection value of version1"""
    injection_parameters_dict = data_ver1.injection_parameters
    ...
    if default_plot_parameters:
        keys_to_plot = others_data['search_parameter_keys']
    elif 'A_b1' in posterior_samples_ver1:
        keys_to_plot = [
                        #'mass_1_source',
                        #'mass_2_source',
                        #'mass_ratio',
    ...
    """fixed parameters"""
    if 'fixed_parameter_keys' in others_data.keys():
        fix_list = others_data['fixed_parameter_keys']
        for key in fix_list:
            print('not plot key : {}'.format(key))

    """extract plot sample"""
    posterior_ver1 = [posterior_samples_ver1[key] for key in keys_to_plot]
    posterior_ver1 = np.array(posterior_ver1).T
    ...
    if show_fig:
        plt.show()
        print('create posterior plot from: {}'.format(path_json))

    else:
        filename = get_filename_without_extension(file_path_ver1)
