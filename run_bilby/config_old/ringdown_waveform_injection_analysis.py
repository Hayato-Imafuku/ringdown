...
def toy_model_of_two_QNMs_real_amp_fomain_mirror(frequency, A, alpha, f1, tau1, f2, tau2, phi1, phi2, geocent_time):
        A = A * 1e-17
        w1 = (2*np.pi*f1 + 1j / tau1)
        w2 = (2*np.pi*f2 + 1j / tau2)
        delta_w = w2 - w1
...
def toy_model_of_two_QNMs_real_amp_fomain_Heaviside(frequency, A, alpha, f1, tau1, f2, tau2, phi1, phi2, geocent_time):
    A = A * 1e-17
    w1 = (2*np.pi*f1 + 1j / tau1)
    w2 = (2*np.pi*f2 + 1j / tau2)
    delta_w = w2 - w1
...
if __name__ == "__main__" :
    ...
    if parameterization == 'OT_real_amp_Mirror' or parameterization == 'OT_real_amp_Heaviside':
        ...
        if parameterization == 'OT_real_amp_Mirror':
            waveform_generator = bilby.gw.waveform_generator.WaveformGenerator(
                                 duration = duration,
                                 sampling_frequency = sampling_frequency,
                                 time_domain_source_model = toy_model_of_two_QNMs_real_amp_fomain_mirror,
                                 start_time = start_time,
                                 )
            ...
            waveform_generator = bilby.gw.waveform_generator.WaveformGenerator(
                                 duration = duration,
                                 sampling_frequency = sampling_frequency,
                                 time_domain_source_model = toy_model_of_two_QNMs_real_amp_fomain_Heaviside,
                                 start_time = start_time,
                                 )
    ...
    if analysis_one_mode:
        ...
        if parameterization == 'DS_window' or parameterization == 'DS_Mirror' or parameterization == 'DS_Heaviside':
            ...
            if injection_parameters['f1'] == injection_parameters['f2']: # f1==f2 --> hierarchical by tau
        ...
        if parameterization == 'OT' or parameterization == 'OT_real_amp':
            ...
            if injection_parameters['f1'] == injection_parameters['f2']: # f1==f2 --> hierarchical by tau
                ...
                priors['f2'] = bilby.core.prior.Uniform(20, 500, name='f2', latex_label=r"$f_2$ [Hz]")
                priors['tau1'] = bilby.core.prior.Triangular(mode=0.05, minimum=0.0005, maximum=0.05, name='tau1', latex_label=r"$\tau_1$ [ms]")
                priors['tau2'] = bilby.core.prior.conditional.ConditionalUniform(condition_func=condition_for_tau2, name='tau2', latex_label=r'$\tau_2$ [ms]', minimum=0.0005, maximum=0.05)
            else: # f1!=f2 --> hierarchical by f
                priors['f1'] = bilby.core.prior.Triangular(mode=500, minimum=20, maximum=500, name='f1', latex_label=r"$f_1$ [Hz]")
                priors['f2'] = bilby.core.prior.conditional.ConditionalUniform(condition_func=condition_for_f2, name='f2', latex_label=r'$f_2$ [Hz]', minimum=20, maximum=500)
                priors['tau1'] = bilby.core.prior.Uniform(0.0005, 0.05, r"$\tau_1$ [ms]")
                priors['tau2'] = bilby.core.prior.Uniform(0.0005, 0.05, name='tau2', latex_label=r"$\tau_2$ [ms]")
